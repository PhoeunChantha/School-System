...
import { KH, Avatar, Badge, ScoreChip } from '../ui';
                ...
                <table className="data-table">
                    ...
                    <tbody>
                        {STUDENTS.filter(s => selClass === 'all' || s.cls === selClass).map(s => {
                            ...
                            return (
                                <tr key={s.id}>
                                    <td><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><Avatar name={s.nameEn} size={30} /><div><KH style={{ fontWeight: 700, fontSize: 12, display: 'block' }}>{s.nameKh}</KH><div style={{ fontSize: 11, color: '#94a3b8' }}>{s.level}</div></div></div></td>
                                    {skills.map(sk => (
                                        <td key={sk}>
                                            {isEd
