...
import type { Screen, Student } from '../data';
import { STUDENTS, CLASSES, avg } from '../data';
import { KH, Avatar, PBar, Badge, FeeTag, ScoreChip } from '../ui';

interface StudentsProps { go: (s: Screen) => void; }

export function StudentsScreen({ go }: StudentsProps) {
    ...
    return (
        <div className="fade-in" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                <input value={search} onChange={e => setSearch(e.target.value)} className="f-input" style={{ maxWidth: 280 }}
                    placeholder="🔍  ស្វែងរក / Search students..." />
                ...
                <button onClick={() => go('addStudent')} style={{ marginLeft: 'auto', background: '#2563eb', color: 'white', border: 'none', borderRadius: 10, padding: '9px 18px', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                    + Add Student
                </button>
            </div>
            ...
            {selected && (
                <div className="card fade-in" style={{ padding: 24 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, minWidth: 240 }}>
                    ...
                    <div style={{ marginTop: 16, padding: '12px 0', borderTop: '1px solid #f1f5f9', display: 'flex', gap: 16, fontSize: 13, color: '#64748b', flexWrap: 'wrap' }}>
...
// ── Add Student ──
interface AddStudentProps { go: (s: Screen) => void; }
export function AddStudentScreen({ go }: AddStudentProps) {
    const [step, setStep] = useState(1);
    const [done, setDone] = useState(false);

    if (done) return (
        <div className="fade-in" style={{ padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400 }}>
            <div style={{ fontSize: 64, marginBottom: 16 }}>✅</div>
            <KH style={{ fontWeight: 800, fontSize: 24, display: 'block', marginBottom: 6 }}>រួចរាល់!</KH>
            <div style={{ color: '#64748b', marginBottom: 24 }}>Student added successfully</div>
            <button onClick={() => go('students')} style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: 12, padding: '12px 32px', fontWeight: 700, cursor: 'pointer' }}>← Back to Students</button>
        </div>
    );
        ...
        <div className="fade-in" style={{ padding: 24 }}>
            <div className="card" style={{ padding: 28, maxWidth: 640, margin: '0 auto' }}>
                ...
                {step === 1 && (
                    <div className="fade-in">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                            <div className="f-group"><label className="f-label">ឈ្មោះ (ខ្មែរ) / Khmer Name *</label><input className="f-input" placeholder="ឧ. សុខ ដារា" /></div>
                            <div className="f-group"><label className="f-label">English Name *</label><input className="f-input" placeholder="e.g. Sokh Dara" /></div>
                ...
                {step === 2 && (
                    <div className="fade-in">
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                            <div className="f-group"><label className="f-label">Level / កម្រិត *</label><select className="f-input">{['Beginner 1', 'Beginner 2', 'Intermediate 1', 'Intermediate 2', 'Advanced 1', 'Advanced 2'].map(l => <option key={l}>{l}</option>)}</select></div>
                            <div className="f-group"><label className="f-label">Class / ថ្នាក់ *</label><select className="f-input">{CLASSES.map(c => <option key={c.id}>{c.name} ({c.time})</option>)}</select></div>
                            <div className="f-group"><label className="f-label">Monthly Fee (USD)</label><input type="number" className="f-input" defaultValue={25} /></div>
                ...
                {step === 3 && (
                    <div className="fade-in">
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                            <div className="f-group"><label className="f-label">Province / ខេត្ត</label><select className="f-input"><option value="">Select...</option>{['Phnom Penh', 'Prey Veng', 'Kampong Cham', 'Kampong Thom', 'Kandal', 'Siem Reap'].map(p => <option key={p}>{p}</option>)}</select></div>
                            ...
                            <div className="f-group"><label className="f-label">Village / ភូមិ</label><input className="f-input" placeholder="Village name..." /></div>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                    ...
                    {step < 3
                        ...
                        : <button onClick={() => setDone(true)} style={{ flex: 2, background: '#10b981', color: 'white', border: 'none', borderRadius: 10, padding: '12px', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: "'Noto Sans Khmer',sans-serif" }}>✓ រក្សាទុក / Save Student</button>
                    }
                </div>
            </div>
        </div>
