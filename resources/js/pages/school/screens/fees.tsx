...
import { KH, Avatar, Badge, FeeTag } from '../ui';
                        ...
                        <tbody>
                            {filtered.map(s => (
                                <tr key={s.id}>
                                    ...
                                    <td>
                                        {s.fees !== 'Paid' && (
                                            <button onClick={() => openModal(s)} style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: 7, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>+ Record</button>
                                        )}
                                    </td>
                                </tr>
                            ))}
                        ...
                        {!done ? (
                            <>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                                    <div>
                                        <KH style={{ fontWeight: 800, fontSize: 18, display: 'block' }}>ទទួលការទូទាត់</KH>
                                        <div style={{ fontSize: 13, color: '#94a3b8' }}>Record Payment · {showModal.nameEn}</div>
                                    </div>
                                ...
                                <div style={{ display: 'flex', gap: 12, alignItems: 'center', background: '#f8fafc', borderRadius: 12, padding: 14, marginBottom: 20 }}>
                                    ...
                                    <div style={{ textAlign: 'right' }}><div style={{ fontSize: 22, fontWeight: 800, color: '#1e293b' }}>${showModal.amt}</div><div style={{ fontSize: 11, color: '#94a3b8' }}>Due</div></div>
                                </div>
                                ...
                                {step === 1 && (
                                    <div>
                                        ...
                                        <button onClick={() => setStep(2)} style={{ width: '100%', background: '#2563eb', color: 'white', border: 'none', borderRadius: 12, padding: '13px', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: "'Noto Sans Khmer',sans-serif" }}>
                                            បន្ត → Next
                                        </button>
                                    </div>
                                )}
                                ...
                                {step === 2 && (
                                    <div>
                                        ...
                                        {method !== 'Cash' && (
                                            <div className="f-group">
                                                ...
                                                <div onClick={() => setScreenshot(true)} style={{ border: `2px dashed ${screenshot ? '#10b981' : '#cbd5e1'}`, borderRadius: 12, padding: 24, textAlign: 'center', cursor: 'pointer', background: screenshot ? '#f0fdf4' : '#f8fafc', transition: 'all 0.2s' }}>
                                                    {screenshot
                                                        ? <><div style={{ fontSize: 32, marginBottom: 6 }}>✅</div><KH style={{ fontWeight: 700, color: '#16a34a', display: 'block' }}>បានបន្ថែម</KH><div style={{ fontSize: 11, color: '#10b981' }}>screenshot_payment.jpg</div></>
                                                        : <><div style={{ fontSize: 32, marginBottom: 6 }}>📱</div><KH style={{ color: '#64748b', display: 'block', fontSize: 13 }}>ចុចដើម្បីផ្ទុករូបភាព</KH><div style={{ fontSize: 11, color: '#94a3b8' }}>Upload payment screenshot</div></>
                                                    }
                                                </div>
                                            </div>
                                        )}
                                        <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                                            ...
                                            <button onClick={() => setDone(true)} style={{ flex: 2, background: '#10b981', color: 'white', border: 'none', borderRadius: 12, padding: '13px', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: "'Noto Sans Khmer',sans-serif" }}>
                                                ✓ Confirm Payment
                                            </button>
                                        </div>
                                    </div>
