...
const STATUS_CONFIG: Record<string, { label: string; badgeClass: string; iconColor: string }> = {
    present: { label: 'Present',  badgeClass: 's-badge-green',  iconColor: '#059669' },
    absent:  { label: 'Absent',   badgeClass: 's-badge-red',    iconColor: '#e11d48' },
    late:    { label: 'Late',     badgeClass: 's-badge-amber',  iconColor: '#d97706' },
    excused: { label: 'Excused',  badgeClass: 's-badge-blue',   iconColor: '#2563eb' },
};
...
function groupByMonth(records: AttendanceRecord[]) {
    ...
    for (const r of records) {
        const key = r.date ? new Date(r.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : 'Unknown';
...
export default function StudentAttendance({ profile, summary, records }: Props) {
    ...
    return (
        <StudentShell profile={profile} activePage="attendance" title="Attendance">
            {/* ── Page header ── */}
            <div className="s-page-header s-fade-up">
                <div className="s-page-accent" style={{ background: '#dcfce7' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                    ...
                    <div className="s-circle-wrap" style={{ flexShrink: 0 }}>
                        ...
                        <div
                            ...
                            style={{ fontSize: 15, fontFamily: 'DM Serif Display, serif', color: '#059669' }}
                        >
                            {summary.rate}%
                        </div>
                    </div>
                    ...
                    <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
                        {[
                            ...
                            { label: 'Absent',  val: summary.absent,  color: '#e11d48' },
                            { label: 'Late',    val: summary.late,    color: '#d97706' },
                            { label: 'Excused', val: summary.excused, color: '#2563eb' },
                        ].map((s) => (
                            <div key={s.label}>
                                <div
                                    style={{
            ...
            {Object.keys(grouped).length === 0 ? (
                <div className="s-card s-fade-up s-delay-3">
                    <div className="s-empty">
                        ...
                        <div className="s-empty-text">No attendance records found</div>
                    </div>
                </div>
            ) : (
                            ...
                            {recs.map((rec, i) => {
                                ...
                                return (
                                    <div key={i} className="s-list-item">
                                        ...
                                        >
                                            {rec.status === 'present' && <CheckCircle size={16} color={cfg.iconColor} />}
                                            {rec.status === 'absent'  && <XCircle    size={16} color={cfg.iconColor} />}
                                            {rec.status === 'late'    && <Clock       size={16} color={cfg.iconColor} />}
                                            {rec.status === 'excused' && <AlertCircle size={16} color={cfg.iconColor} />}
                                            {!['present','absent','late','excused'].includes(rec.status) && (
                                                <CalendarCheck size={16} color={cfg.iconColor} />
                                            )}
                                        </div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
                                            <span className={`s-badge ${cfg.badgeClass}`}>{cfg.label}</span>
