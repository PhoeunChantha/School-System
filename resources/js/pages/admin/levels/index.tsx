            ...
            <div className="fade-in" style={{ padding: 24 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                    ...
                    {view === 'list'
                        ...
                        : <button onClick={() => setView('list')} style={{ background: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: 10, padding: '10px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>← Back</button>
                    }
                </div>
                ...
                {(view === 'add' || view === 'edit') && (
                    <div className="card" style={{ padding: 24, maxWidth: 480, marginBottom: 24 }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: '#1e293b', marginBottom: 18 }}>
                            {view === 'edit' ? `Edit — ${editing?.name}` : 'Add New Level'}
                        </div>
                        <form onSubmit={handleSubmit}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                                ...
                                <div className="f-group" style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <input
                                        type="checkbox"
                                        id="is_active"
                            ...
                            <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
                                <button type="button" onClick={() => setView('list')} style={{ flex: 1, background: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: 10, padding: '11px', fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
                                <button type="submit" disabled={processing} style={{ flex: 2, background: '#2563eb', color: 'white', border: 'none', borderRadius: 10, padding: '11px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                                    {processing ? 'Saving...' : view === 'edit' ? 'Update Level' : 'Save Level'}
                                </button>
                            </div>
                        </form>
                    </div>
